...
def inception_module(l_in,pool_filters, num_1x1, reduce_3x3, num_3x3, reduce_5x5, num_5x5, bias=0):
    ...
    if pool_filters > 0:
        ...
        l_pool_reduced = lasagne.layers.NINLayer(l_pool, num_units=pool_filters, W=lasagne.init.HeNormal(gain='relu'), b=lasagne.init.Constant(bias))
        out_layers.append(l_pool_reduced)

    # 1x1
    if num_1x1 > 0:
        l_1x1 = lasagne.layers.NINLayer(l_in, num_units=num_1x1, W=lasagne.init.HeNormal(gain='relu'), b=lasagne.init.Constant(bias))
        out_layers.append(l_1x1)
    
    # 3x3
    if num_3x3 > 0:
        if reduce_3x3 > 0:
            l_reduce_3x3 = lasagne.layers.NINLayer(l_in, num_units=reduce_3x3,W=lasagne.init.HeNormal(gain='relu'), b=lasagne.init.Constant(bias))
        else:
            l_reduce_3x3 = l_in
        l_3x3 = Conv2DLayer(l_reduce_3x3, num_filters=num_3x3, filter_size=(3, 3), pad="same", W=lasagne.init.HeNormal(gain='relu'), b=lasagne.init.Constant(bias))
        out_layers.append(l_3x3)
    
    # 5x5
    if num_5x5 > 0:
        if reduce_5x5 > 0:
            l_reduce_5x5 = lasagne.layers.NINLayer(l_in, num_units=reduce_5x5, W=lasagne.init.HeNormal(gain='relu'), b=lasagne.init.Constant(bias))
        else:
            l_reduce_5x5 = l_in
        l_5x5 = Conv2DLayer(l_reduce_5x5, num_filters=num_5x5, filter_size=(5, 5), pad="same", W=lasagne.init.HeNormal(gain='relu'), b=lasagne.init.Constant(bias))
...
def build_cnn(input_var=None):
    ...
    network = lasagne.layers.NINLayer(network, num_units=32, W=lasagne.init.HeNormal(gain='relu'), b=lasagne.init.Constant(0))
...
def main(num_epochs=200):
    ...
    loss = loss.mean()+0.01*l1_penalty
